# Chainalysis KYT Screener

> **Moniversive Invariant Static (MIS)** module · settles on **CLRTY-1** (chain **1202**) · compiler **`misc`**  
> Creator **Chandler William Ferguson** · Org [`clarity-fintech/chainalysis-kyt-screener`](https://github.com/clarity-fintech/chainalysis-kyt-screener)
...
**Chainalysis KYT Screener** is written entirely in **MIS** (Moniversive Invariant Static, `.mis`) — a
...
```bash
git clone https://github.com/clarity-fintech/chainalysis-kyt-screener
cd chainalysis-kyt-screener
...
| `bin/misc mis/kernel/ChainalysisKytScreenerKernel.mis --check --compact-letters` | Check a single module directly with the compiler. |

Direct compiler invocation (what the Makefile runs under the hood):

```bash
bin/misc mis/kernel/ChainalysisKytScreenerKernel.mis --check --compact-letters
...
```
mis/kernel/ChainalysisKytScreenerKernel.mis
mis/packs/clrty1_attest.mis
mis/packs/flow_trace.mis
mis/packs/illicit_flag.mis
mis/packs/kyt_score.mis
mis/packs/risk_band.mis
...
mis/commands/ChainalysisKytScreenerCommandCatalog.mis
mis/commands/ChainalysisKytScreenerCommands.mis
mis/commands/ChainalysisKytScreenerNetworkBind.mis
...
`mis/kernel/ChainalysisKytScreenerKernel.mis` — the kernel declares the invariants the compiler enforces on every check:

```rust
// Moniversive Invariant Static (MIS) — deep root
// Creator: Chandler William Ferguson
// Letter-hash + EMBED gates 3..=6 · settlement clrty-1 / chain 1202
// Compile: bin/misc <file>.mis --check --compact-letters
// Title: Chainalysis KYT Screener Kernel

module ChainalysisKytScreenerKernel {
  ...
  invariant domain_bound: domain == chainalysis_kyt_screener;
  ...
  invariant pack_kyt_score_flag: pack_kyt_score_bound == true;
  invariant pack_illicit_flag_flag: pack_illicit_flag_bound == true;
  invariant pack_flow_trace_flag: pack_flow_trace_bound == true;
  invariant pack_risk_band_flag: pack_risk_band_bound == true;
  invariant pack_clrty1_attest_flag: pack_clrty1_attest_bound == true;
```
...
// Letter-hash + EMBED gates 3..=6 · settlement clrty-1 / chain 1202
// Compile: bin/misc <file>.mis --check --compact-letters
// Title: Chainalysis KYT Screener · Clrty1Attest

module ChainalysisKytScreenerClrty1Attest {
  ...
  invariant settlement_network_clrty1: settlement_network == clrty_1;
  invariant kernel_is_misc: compiler_kernel == misc;
  invariant pack_clrty1_attest_bound: pack_bound == true;
  invariant catalog_backlink: mis_catalog_bound == true;

  invariant clrty1_attest_081: clrty1_attest_081_bound == true;
  invariant clrty1_attest_082: clrty1_attest_082_bound == true;
  invariant clrty1_attest_083: clrty1_attest_083_bound == true;
  invariant clrty1_attest_084: clrty1_attest_084_bound == true;
  invariant clrty1_attest_085: clrty1_attest_085_bound == true;
  invariant clrty1_attest_086: clrty1_attest_086_bound == true;
```
...
```
CONNECTED clrty-1/1202 chainalysis-kyt-screener
RPC https://rpc.clarity-fintech.com
MODULES mis/commands/ChainalysisKytScreenerCommands.mis
```
...
{
  "ok": true,
  "module": "ChainalysisKytScreenerBacklinkIndex",
  "kernel": "misc",
  "invariant_count": 9,
  "outcome_count": 4,
  "typed_letters": 1239,
...
*Reproduce:* `make check` → `"ok": true`, module `ChainalysisKytScreenerBacklinkIndex`, 9 invariants, 4 outcomes, 1239 typed letters.
...
- [clarity-fintech/chainalysis-kyt-screener](https://github.com/clarity-fintech/chainalysis-kyt-screener) — this repository
