...
check:
	...
	$(MISC) clrty/chainalysis-kyt-screener.mis --check --compact-letters
	$(MISC) mis/clrty1_e2e_bind.mis --check --compact-letters
	$(MISC) mis/kernel/ChainalysisKytScreenerKernel.mis --check --compact-letters
	@find mis -name '*.mis' | while read f; do $(MISC) "$$f" --check --compact-letters || exit 1; done
	@echo OK chainalysis-kyt-screener
...
network-connect: commands-check
	...
	@echo MODULES mis/commands/ChainalysisKytScreenerCommands.mis
